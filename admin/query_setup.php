...
print $formCore->texte('', 'QUERY_WKHTMLTOPDF_CMD', empty($conf->global->ABRICOT_WKHTMLTOPDF_CMD) ? '' : $conf->global->ABRICOT_WKHTMLTOPDF_CMD, 80,255,' placeholder="wkhtmltopdf" ');
print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
print '</form>';
print '</td></tr>';
...
$formCore=new TFormCore;
print $formCore->texte('', 'ABRICOT_CONVERTPDF_CMD', empty($conf->global->ABRICOT_CONVERTPDF_CMD) ? '' : $conf->global->QUERY_WKHTMLTOPDF_CMD, 80, 255,' placeholder="libreoffice --invisible --norestore --headless --convert-to pdf --outdir " ');
print '<input type="submit" class="button" value="'.$langs->trans("Modify").'">';
print '</form>';
print '</td></tr>';
