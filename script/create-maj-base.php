...
// on vérifie que la table est vide pour eviter la fatal DUPLICATE PRIMARY KEY (PHP8)
$PDOdb->Execute("SELECT COUNT(rowid) as nb FROM ".MAIN_DB_PREFIX."qdashboard");
$obj = $PDOdb->Get_line();
if ($obj->nb == 0){
	$PDOdb->Execute("INSERT INTO ".MAIN_DB_PREFIX."qdashboard (`rowid`, `date_cre`, `date_maj`, `fk_user`, `fk_user_author`, `fk_usergroup`, `uid`, `send_by_mail`, `hook`, `title`, `refresh_dashboard`) VALUES
...
$PDOdb->Execute("SELECT COUNT(rowid) as nb FROM ".MAIN_DB_PREFIX."qdashboard_query");
$obj = $PDOdb->Get_line();
if ($obj->nb == 0) {
	$PDOdb->Execute("INSERT INTO ".MAIN_DB_PREFIX."qdashboard_query (`rowid`, `date_cre`, `date_maj`, `fk_qdashboard`, `fk_query`, `width`, `height`, `posx`, `posy`, `title`) VALUES
...
$PDOdb->Execute("SELECT COUNT(rowid) as nb FROM ".MAIN_DB_PREFIX."query");
$obj = $PDOdb->Get_line();
if ($obj->nb == 0) {
	$PDOdb->Execute("INSERT INTO ".MAIN_DB_PREFIX."query (`rowid`, `date_cre`, `date_maj`, `sql_fields`, `sql_from`, `sql_where`, `sql_afterwhere`, `TField`, `TTable`, `TOrder`, `TTitle`, `TLink`, `THide`, `TTranslate`, `TMode`, `TOperator`, `TGroup`, `TFunction`, `TValue`, `TJoin`, `expert`, `title`, `type`, `xaxis`) VALUES
